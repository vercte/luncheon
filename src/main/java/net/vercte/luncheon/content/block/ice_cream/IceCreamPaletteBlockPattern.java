...
import static net.vercte.luncheon.content.block.ice_cream.IceCreamPaletteBlockPartial.ALL_PARTIALS;
import static net.vercte.luncheon.content.block.ice_cream.IceCreamPaletteBlockPartial.FOR_POLISHED;
...
public class IceCreamPaletteBlockPattern {
	...
	LAYERED = create("layered", PatternNameType.PREFIX, new IceCreamPaletteBlockPartial[]{}).blockStateFactory(p -> p::cubeColumn)
			.textures("layered", "cap")
			.connectedTextures(v -> new HorizontalCTBehaviour(ct(v, CTs.LAYERED), ct(v, CTs.CAP))),

	PILLAR = create("pillar", PatternNameType.SUFFIX, new IceCreamPaletteBlockPartial[]{}).blockStateFactory(p -> p::pillar)
	...
	public static final IceCreamPaletteBlockPattern[] VANILLA_RANGE = { CUT, POLISHED, BRICKS, SMALL_BRICKS, LAYERED, PILLAR };

	public static final IceCreamPaletteBlockPattern[] STANDARD_RANGE = { CUT, POLISHED, BRICKS, SMALL_BRICKS, LAYERED, PILLAR };
	...
	public IceCreamPaletteBlockPartial<? extends Block>[] getPartials() {
		return partials;
	}
	...
	private IceCreamPaletteBlockPattern blockStateFactory(IPatternBlockStateGenerator factory) {
		blockStateGenerator = factory;
		return this;
	}

	private IceCreamPaletteBlockPattern textures(String... textures) {
		this.textures = textures;
		return this;
	}

	private IceCreamPaletteBlockPattern block(NonNullFunction<Properties, ? extends Block> blockFactory) {
		this.blockFactory = blockFactory;
		return this;
	}

	private IceCreamPaletteBlockPattern connectedTextures(Function<String, ConnectedTextureBehaviour> factory) {
	...
	protected String createName(String variant) {
		...
		return nameType == PatternNameType.SUFFIX ?
				String.format(formatString, variant, id) :
				String.format(formatString, id, variant);
	}

	protected static ResourceLocation toLocation(String variant, String texture) {
		return Luncheon.asResource(
	...
	static interface IPatternBlockStateGenerator
			extends Function<IceCreamPaletteBlockPattern, Function<String, IBlockStateProvider>> {
	}
